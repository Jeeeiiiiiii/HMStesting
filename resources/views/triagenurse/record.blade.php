        ...
        <div class="lg:col-span-1 bg-white p-4 rounded-md shadow-md border border-gray-200">
            ...
            <div class="p-4 space-y-6">
                ...
                <div>
                    ...
                    <button id="myBtn{{ $patient->id }}" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50" data-record-id="{{ $patient->id }}">Open QR</button>
        ...
        <div class="lg:col-span-2 bg-white p-4 rounded-md shadow-md border border-gray-200">
            ...
            <div class="p-4 space-y-6">
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-800">{{ $patient->physical_assessment->general_appearance ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">Pain Assessment</p>
                    <p class="text-lg font-semibold text-gray-800">{{ $patient->physical_assessment->pain_assessment ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">Pain Description</p>
                    <p class="text-lg font-semibold text-gray-800">{{ $patient->physical_assessment->pain_description ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">Changes in Description</p>
                    <p class="text-lg font-semibold text-gray-800">{{ $patient->physical_assessment->changes_in_condition ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">Assessment Date</p>
                    <p class="text-lg font-semibold text-gray-800">{{ $patient->physical_assessment->assessment_date ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-sm font-medium text-gray-500">Assessed by</p>
                    <p class="text-lg font-semibold text-gray-800">{{ $patient->physical_assessment->nurse->name ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
    </div>
...
function openModal(modalId, patientRecordId) {
    ...
    if (modal) {
        ...
        if (qrCodeImg) {
            ...
            fetch(`/triagenurse/record/qr-code/${patientRecordId}`) // Update the endpoint URL as necessary
