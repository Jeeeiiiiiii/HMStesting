    ...
    <div class="grid grid-cols-1 lg:grid-cols-1 gap-6">
        ...
        <div class="bg-white border border-gray-200 shadow-md p-6 rounded-lg mt-4">
            ...
            <div class="flex flex-col md:flex-row mb-4">
                ...
                <div class="font-bold text-2xl text-gray-800">{{ $triagenurse->email }}</div>
            </div>
            <hr class="my-4 border-gray-300">
            <div class="flex flex-col md:flex-row mb-4">
                <div class="w-full md:w-52 mb-2 md:mb-0">
                ...
                <div class="font-bold text-2xl text-gray-800">{{ $triagenurse->profile->telephone_no ?? 'N/A' }}</div>
            </div>
            <hr class="my-4 border-gray-300">
            ...
            <div class="flex flex-col md:flex-row mb-4">
                ...
                <div class="font-bold text-2xl text-gray-800">{{ $triagenurse->profile->emergency_email ?? 'N/A' }}</div>
            </div>
            <hr class="my-4 border-gray-300">
            <div class="flex flex-col md:flex-row mb-4">
                <div class="w-full md:w-52 mb-2 md:mb-0">
                ...
                <div class="font-bold text-2xl text-gray-800">{{ $triagenurse->profile->emergency_telephone_no ?? 'N/A' }}</div>
            </div>
        </div>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
    <div class="relative mx-auto p-6 border w-96 shadow-xl rounded-md bg-white mt-20 top-1/3 transform -translate-y-1/2">
        <div class="mt-3 text-center">
            ...
            <div class="mt-4">
                <form action="{{ route('triagenurse_submitinfo') }}" method="POST">
                    @csrf

                    <!-- Form fields -->
                    <div class="mb-4">
                        <label for="email" class="block text-sm font-medium text-gray-700">Email Address</label>
                        <input type="email" name="email" id="email" value="{{ $triagenurse->email }}" class="mt-1 block w-full p-2 border rounded-md shadow-sm">
                    </div>
                    <div class="mb-4">
                        <label for="telephone_no" class="block text-sm font-medium text-gray-700">Contact Number</label>
                        <input type="text" name="telephone_no" id="telephone_no" value="{{ $triagenurse->profile->telephone_no }}" class="mt-1 block w-full p-2 border rounded-md shadow-sm">
                    </div>
...
<div id="editModal2" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
    <div class="relative mx-auto p-6 border w-96 shadow-xl rounded-md bg-white mt-20 top-1/3 transform -translate-y-1/2">
        <div class="mt-3 text-center">
            ...
            <div class="mt-4">
                <form action="{{ route('triagenurse_submitemergencyinfo') }}" method="POST">
                    @csrf

                    <!-- Form fields -->
                    <div class="mb-4">
                        <label for="emergency_email" class="block text-sm font-medium text-gray-700">Emergency Contact Email</label>
                        <input type="email" name="emergency_email" id="emergency_email" value="{{ $triagenurse->profile->emergency_email ?? 'N/A' }}" class="mt-1 block w-full p-2 border rounded-md shadow-sm">
                    </div>
                    <div class="mb-4">
                        <label for="emergency_telephone_no" class="block text-sm font-medium text-gray-700">Emergency Contact Number</label>
                        <input type="text" name="emergency_telephone_no" id="emergency_telephone_no" value="{{ $triagenurse->profile->emergency_telephone_no ?? 'N/A' }}" class="mt-1 block w-full p-2 border rounded-md shadow-sm">
                    </div>
