...
<div class="p-6">
    <div class="grid grid-cols-1 gap-6 mb-6">
        <div class="bg-white border border-gray-100 shadow-md shadow-black/5 p-6 rounded-md">
            <div class="flex mb-4">
                <div class="font-medium text-lg text-gray-700">Department Details</div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                ...
                <!-- Doctor/Nurse Filter -->
                <form action="{{ route('department_details', ['id' => $department->id]) }}" method="GET" class="flex flex-col sm:flex-row w-full sm:w-auto gap-4">
                    <div>
                ...
                <div class="relative w-full sm:w-auto">
                    <button class="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md text-sm px-4 py-2 focus:ring-2 focus:ring-blue-300 transition duration-200 block text-center" id="assignButton" onclick="toggleDropdown()">Assign Staff</button>
                    <div id="dropdownMenu" class="absolute hidden bg-white shadow-md rounded-lg mt-2">
                        <a href="{{ route('assign-doctor.create') }}" class="block px-4 py-2 text-blue-600 hover:bg-gray-100">Assign Doctor</a>
                        <a href="{{ route('assign-nurse.create') }}" class="block px-4 py-2 text-green-600 hover:bg-gray-100">Assign Nurse</a>
                        <a href="{{ route('assign-triage_nurse.create') }}" class="block px-4 py-2 text-red-600 hover:bg-gray-100">Assign Triage Nurse</a>
                    </div>
                </div>
            </div>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="py-2 px-4 border-b border-gray-200">
                                <div class="flex gap-2">
                                   <a href="{{ request('status', 'doctor') == 'doctor' ? route('admin_doctors_profile', $person->id) : (request('status') == 'nurse' ? route('admin_nurses_profile', $person->id) : route('admin_triage_nurses_profile', $person->id)) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg text-sm px-3 py-2 focus:ring-2 focus:ring-blue-300 transition duration-200">Detail</a>
                                    <!-- Form for removing staff -->
                                    <form action="{{ route(request('status', 'doctor') == 'doctor' ? 'doctors.remove' : (request('status') == 'nurse' ? 'nurses.remove' : 'triage_nurses.remove'), ['departmentId' => $department->id, request('status', 'doctor') == 'doctor' ? 'doctorId' : (request('status') == 'nurse' ? 'nurseId' : 'triagenurseId') => $person->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to remove this {{ request('status', 'doctor') == 'doctor' ? 'doctor' : (request('status') == 'nurse' ? 'nurse' : 'triage nurse') }} from the department?')">
                                        @csrf
                                        @method('POST')
