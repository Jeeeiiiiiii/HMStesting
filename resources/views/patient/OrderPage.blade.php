    ...
    <div class="bg-white rounded-lg shadow-md w-full max-w-4xl">
        ...
        <div class="p-6">
            ...
            <div class="mb-4">
                ...
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $order->title }}</p>
            </div>

            <!-- Order Type -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Order Type</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $order->type }}</p>
            </div>

            <!-- Order Description -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Description</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $order->description }}</p>
            </div>

            <!-- Order Status -->
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Status</label>
                <p class="bg-gray-50 text-sm text-gray-600 py-2 px-4 rounded-md w-full">{{ $order->status }}</p>
            </div>
             ...
             <div class="mb-4">
                ...
                @php
                    $orderDate = new DateTime($order->order_date); // Convert string to DateTime object
                @endphp
            ...
            <div>
                ...
                <button id="myBtn{{ $order->id }}" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50" data-order-id="{{ $order->id }}">Open QR</button>

                <!-- The Modal -->
                <div id="myModal{{ $order->id }}" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-auto">
                    <!-- Modal content -->
                    <div class="relative bg-white rounded-lg mx-auto mt-20 p-6 w-11/12 md:w-1/3">
                        <div class="flex items-center justify-between border-b border-gray-200 pb-2 mb-4">
                            ...
                            <button class="text-gray-600 hover:text-gray-800 close" id="closeBtn{{ $order->id }}">&times;</button>
                        </div>
                        <div class="mb-4 text-center"> <!-- Added text-center class -->
                            <p class="text-gray-600 mb-4">Scan to show details</p>
                            <!-- QR Code Image -->
                            <img id="qrCodeImg{{ $order->id }}" src="" alt="QR Code" class="mx-auto"> <!-- Added mx-auto class -->
                        </div>     
                        <!-- Print Button -->
                        <div class="text-center">
                            <button onclick="printQRCode('{{ $order->id }}')" class="py-1 px-3 border border-gray-400 text-gray-700 bg-gray-200 rounded hover:bg-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-300">
                                Print
                            </button>
                        </div>                 
                    </div>
...
function openModal(modalId, patientRecordId) {
    ...
    if (modal) {
        ...
        if (qrCodeImg) {
            ...
            fetch(`/patient/order/qr-code/${patientRecordId}`) // Update the endpoint URL as necessary
